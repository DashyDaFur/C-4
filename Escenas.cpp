...
bool gameOver;
int winner;
int hoverColumn; // Para el feedback visual del rat�n (opcional, se podr�a quitar)
...
// y hoverColumn para un feedback visual secundario del rat�n si se desea.
void DrawHoverAndSelectedPiece()
{
    ...
    // Dibujar la ficha de previsualizaci�n seleccionada por teclado
    if (columnToDrawPreview != -1)
    {
        ...
        (
            ...
            0.0f,
            scale,
            WHITE // Ficha s�lida para la selecci�n por teclado
        );
    }

    // Opcional: Dibujar una ficha "fantasma" m�s tenue para el hover del rat�n, si es diferente de la selecci�n del teclado
    if (!gameOver && hoverColumn != -1 && hoverColumn != selectedColumnKeyboard)
    {
        ...
        (
            ...
            0.0f,
            scaleMouse,
            Fade(WHITE, 0.5f) // Semi-transparente para el hover del rat�n
        );
    }
}
